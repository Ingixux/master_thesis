...
class Entropy:
    def __init__(self,sliding_window_interval,aggregate_window_duration,comparison_window_interval,limited):
        ...
        for x in range(0,int(aggregate_window_duration/sliding_window_interval)):
            self.aggregate_window["vaules"].append({"sourceIP":{},"packets":0,"destinationIP":{},"bytes":0,"packetsize":{},"currentRecs":[],"biflow":{}})
    ...
    def addVaules(self,rec):
        ...
        self.aggregate_window["vaules"][0]["bytes"]=self.aggregate_window["vaules"][0]["bytes"] +rec.bytes #TODO fix this

        checkKeyOfBiFlow = str(rec.dip)+ str(rec.sip) + str(rec.dport) +str(rec.sport) +str(rec.protocol)
        if checkKeyOfBiFlow in self.aggregate_window["vaules"][0]["biflow"].keys():
            self.aggregate_window["vaules"][0]["biflow"][checkKeyOfBiFlow]["packets"]+=1*rec.packets
        else:
    ...
    def doCalculation(self):
        ...
        for x in range(0,len(self.aggregate_window["vaules"])):
            ...
            for key in self.aggregate_window["vaules"][x]["packetsize"].keys():
                try:
                    ...
                    uniqepacketsize[key]=self.aggregate_window["vaules"][x]["packetsize"][key]
            for key in self.aggregate_window["vaules"][x]["biflow"].keys(): 
                try:
                    uniqebiflow[key]+=self.aggregate_window["vaules"][x]["biflow"][key]["packets"]
                except KeyError:
                    try:
                        uniqebiflow[self.aggregate_window["vaules"][x]["biflow"][key]["checkKey"]]+=self.aggregate_window["vaules"][x]["biflow"][key]["packets"]
                    except KeyError:
                        uniqebiflow[key]=self.aggregate_window["vaules"][x]["biflow"][key]["packets"]
        ...
        listofprobability=[]
        for x in uniqebiflow.values():
            listofprobability.append(x/totalpackets)
        entropyBiflow=self.entropyCal(listofprobability)
        entropyRateBiflowe=entropyPacketsize/len(uniqebiflow)
        ...
        for rec in self.aggregate_window["vaules"][-1]["currentRecs"]:
            arrayToAdd.append([rec.stime, rec.etime, int(rec.sip), int(rec.dip), rec.sport, rec.dport, rec.protocol, rec.packets, rec.bytes, 
                                ...
                                entropySip,entropyRateSip,entropyDip,entropyRateDip,entropyPacketsize,entropyRatePacketsize,entropyBiflow,entropyRateBiflowe,
    ...
    def removeFromSliding_window(self):
        ...
        self.aggregate_window["vaules"][0]={"sourceIP":{},"packets":0,"destinationIP":{},"bytes":0,"packetsize":{},"currentRecs":[],"biflow":{}}
